...
QVector<Token> CppTokenizer::tokenizeCode(const QString &code)
{
    ...
    while (i < code.length()) {
        ...
        if (c == ' ') {
            ...
            tokens.append({preproc, "preprocessor"});
        } else if (c == '{' || c == '}' || c == '[' || c == ']' || c == '(' || c == ')') {
            ...
            i++;
            while (i < code.length() && (code[i].isDigit() || code[i] == '.' || code[i] == 'f' || code[i] == 'F')) {
                num += code[i];
                i++;
            }
